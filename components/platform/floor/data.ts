/**
 * The morning the Floor models.
 *
 * Lifted wholesale from the first `components/platform/agent-floor.tsx`, which
 * this directory replaces. The scene changed language completely; the morning
 * did not, and none of it is invented: every figure here is one a venue on
 * these systems would actually see, and the wording has already been through a
 * copy review against the budgets in CLAUDE.md. Change the art, not the facts.
 *
 * The venue's own model is a running Friday service rather than a date picker.
 * A clock drives it: tables count down to their booking, fill when it lands and
 * clear when the turn is up, while the phone keeps answering in the margin.
 * Paging through days showed the same information as a filmstrip; a running
 * clock shows it as a shift, which is what an owner recognises.
 */

import type { FloorState } from "./palette";
...
/** The minute the brief lands on the desk, and the minute the page opens on. */
export const BRIEF = 6 * 60 + 4;

export const OPEN = 17 * 60 + 30; // 17:30, doors

/**
 * Where the clock starts when a reader steps into the room.
 *
 * Not at the door, and not at the peak either. At open every table is free and
 * the phone has answered nothing; at the top of the first turn every table is
 * seated or about to be, so nothing advances and §11's sixth check has nothing
 * to measure. 19:40 is the moment the room says most: six tables seated and
 * receded, two turning, two free and forward, and four calls already handled.
 */
export const ENTER_AT = 19 * 60 + 40;
...
/**
 * How far ahead a booking has to be before the table counts as free.
 *
 * A table with nobody on it and nobody due for five hours is free, and drawing
 * it as half-booked made the whole room read as pale mush at six in the
 * morning, which is the hour the page opens on.
 */
export const DUE_WINDOW = 30;
...
export function tableState(id: string, now: number): TableState {
  ...
  const next = mine.find((s) => s.at > now && s.at - now <= DUE_WINDOW);
